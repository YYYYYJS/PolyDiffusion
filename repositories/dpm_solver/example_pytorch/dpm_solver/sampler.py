...
def model_wrapper(model, noise_schedule=None, is_cond_classifier=False, classifier_fn=None, classifier_scale=1., time_input_type='1', total_N=1000, model_kwargs={}):
    """Create a wrapper function for the noise prediction generator.

    DPM-Solver needs to solve the continuous-time diffusion ODEs. For DPMs trained on discrete-time labels, we need to
    firstly wrap the generator function to a function that accepts the continuous time as the input.

    The input `generator` has the following format:

    ``
        generator(x, t_input, **model_kwargs) -> noise
    ``

    where `x` and `noise` have the same shape, and `t_input` is the time label of the generator.
    (may be discrete-time labels (i.e. 0 to 999) or continuous-time labels (i.e. epsilon to T).)

    We wrap the generator function to the following format:

    ``
        def model_fn(x, t_continuous) -> noise:
            t_input = get_model_input_time(t_continuous)
            return generator(x, t_input, **model_kwargs)
    ``
    
    where `t_continuous` is the continuous time labels (i.e. epsilon to T). And we use `model_fn` for DPM-Solver.

    For DPMs with classifier guidance, we also combine the generator output with the classifier gradient as used in [1].
    ...
    Args:
        model: A noise prediction generator with the following format:
            ``
                def generator(x, t_input, **model_kwargs):
                    return noise
            ``
        ...
        time_input_type: A `str`. The type for the time input of the generator. We support three types:
            - '0': The continuous-time type. In this case, the generator is trained on the continuous time,
        ...
        total_N: A `int`. The total number of the discrete-time DPMs (default is 1000), used when `time_input_type`
            is '1' or '2'.
        model_kwargs: A `dict`. A dict for the other inputs of the generator function.
    Returns:
    ...
    def get_model_input_time(t_continuous):
        """
        Convert the continuous-time `t_continuous` (in [epsilon, T]) to the generator input time.
        """
        if time_input_type == '0':
            # discrete_type == '0' means that the generator is continuous-time generator.
    ...
    def model_fn(x, t_continuous):
        """
        The noise predicition generator function that is used for DPM-Solver.
        """
...
class DPM_Solver:
    def __init__(self, model_fn, noise_schedule):
        ...
        Args:
            model_fn: A noise prediction generator function which accepts the continuous-time input
                (t in [epsilon, T]):
                ``
                def model_fn(x, t_continuous):
                    return noise

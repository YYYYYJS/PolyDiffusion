...
@torch.no_grad()
def layout_to_image_generation(cfg, model_fn, noise_schedule, custom_layout_dict,text):
    ...
    x_T = pipe(prompt=text, height=16, width=16)
...
def init():
    ...
    cfg.sample.pretrained_model_path = 'G:/contour_weight_128/ContourDiffusion0021216.pt'
...
if __name__ == "__main__":
    ...
    with open('text_dict.pkl', 'rb') as file:
        texts = pickle.load(file)

    for i in range(custom_layout_dict['obj_bbox'].shape[0]):
        cond = {
            'obj_description': custom_layout_dict['obj_description'][i:i + 1, :],
            'obj_contour': custom_layout_dict['obj_contour'][i:i + 1, :, :, :],
            'is_valid_obj': custom_layout_dict['is_valid_obj'][i:i + 1, :],
            'mask': custom_layout_dict['mask'][i:i + 1, :, :],
            'obj_bbox': custom_layout_dict['obj_bbox'][i:i + 1, :, :]
        }
        text = texts[i]
        print(text)
        ...
        img = layout_to_image_generation(cfg, model_fn, noise_schedule, cond,text)
